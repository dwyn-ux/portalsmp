...
<body class="bg-gray-50 text-gray-900 antialiased min-h-screen" x-data="{ sidebarOpen: true, mobileMenu: false }">
    <nav class="fixed top-0 left-0 right-0 z-50 bg-white border-b border-gray-200 h-16">
        <div class="flex items-center justify-between h-full px-4 lg:px-6">
            ...
            <div class="flex items-center gap-3">
                <a href="/" target="_blank" class="text-sm text-gray-500 hover:text-green-600 transition flex items-center gap-1">
                ...
                <div class="flex items-center gap-2">
                    <div class="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center">
                        <span class="text-sm font-medium text-green-700"><?= \App\Helpers\H::e(substr($_SESSION['user']['name'] ?? 'A', 0, 1)) ?></span>
                    </div>
    ...
    <div class="flex pt-16">
        <aside class="fixed left-0 top-16 bottom-0 w-64 bg-white border-r border-gray-200 overflow-y-auto z-40 hidden lg:block"
            ...
            <div class="p-4 space-y-1">
                ...
                <?php foreach ($menuItems as $item): ?>
                    ...
                    <a href="<?= $item['url'] ?>"
                       class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 <?= $isActive ? 'bg-green-50 text-green-700 shadow-sm' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' ?>">
                        <svg class="w-5 h-5 flex-shrink-0 <?= $isActive ? 'text-green-600' : 'text-gray-400' ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <?= $item['icon'] ?>
                        </svg>
                        <?= $item['label'] ?>
                    </a>
        ...
        <aside x-show="mobileMenu" x-transition:transition="transition transform ease-out duration-300"
            ...
            <div class="p-4 space-y-1">
                <?php foreach ($menuItems as $item): ?>
                    <?php $isActive = rtrim($currentPath, '/') === rtrim($item['url'], '/'); ?>
                    <a href="<?= $item['url'] ?>"
                       class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all <?= $isActive ? 'bg-green-50 text-green-700' : 'text-gray-600 hover:bg-gray-50' ?>">
                        <svg class="w-5 h-5 <?= $isActive ? 'text-green-600' : 'text-gray-400' ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <?= $item['icon'] ?>
                        </svg>
                        <?= $item['label'] ?>
                    </a>
        ...
        <main class="flex-1 lg:ml-64 min-h-screen p-4 lg:p-8">
            <?= $content ?>
        </main>
    </div>
