...
$sekolah = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM kp_pengaturan WHERE id = 1"));
$user = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM kp_users WHERE id = 1"));
...
if (isset($_POST['simpan'])) {
	...
	$nip_kepala  = mysqli_real_escape_string($conn, $_POST['nip_kepala_sekolah']);
    $tanggal = mysqli_real_escape_string($conn, $_POST['tanggal_ttd']);
...
if (!empty($_POST['password_lama']) || !empty($_POST['password_baru']) || !empty($_POST['konfirmasi_password'])) {
    ...
    mysqli_query($conn, "UPDATE kp_users SET password = '$password_md5' WHERE id = 1");
}
    ...
    $update = mysqli_query($conn, "UPDATE kp_pengaturan SET
        nama_sekolah = '$nama',
        alamat = '$alamat',
