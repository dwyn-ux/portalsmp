...
class PDF_Kartu extends FPDF {
    ...
    function gambarDepan($x, $y, $data, $kp_pengaturan) {
        $this->SetXY($x, $y);
        $this->Rect($x, $y, 86, 54);
        $this->SetFont('Arial','B',10);
        
         // Background dinamis dari database
    $background_path = 'assets/background/' . $kp_pengaturan['background'];
    if (file_exists($background_path)) {
        $this->Image($background_path, $x, $y, 86, 54);
    }


        $logo_path = 'assets/logo/' . $kp_pengaturan['logo'];
        ...
        $this->Cell(60, 5, strtoupper($kp_pengaturan['nama_sekolah']), 0, 1, 'L');
    ...
    function gambarBelakang($x, $y, $data, $kp_pengaturan) {
		
         // Background dinamis dari database
    $background_path = 'assets/background_belakang/' . $kp_pengaturan['background_belakang'];
    if (file_exists($background_path)) {
        $this->Image($background_path, $x, $y, 86, 54);
    }
        ...
        $this->MultiCell(75, 5, 
            ...
            $kp_pengaturan['nama_sekolah'] . "\n" .
            $kp_pengaturan['alamat'], 0, 'L');

		// Tanggal dinamis dari database kp_pengaturan
		$tanggal_ttd = tanggal_indonesia($kp_pengaturan['tanggal_ttd']);
		$this->SetXY($x + 2, $y + 30);
        ...
        $ttd_path = 'assets/tanda_tangan/' . $kp_pengaturan['tanda_tangan'];
        if (file_exists($ttd_path)) {
            $this->Image($ttd_path, $x + 50, $y + 37, 30, 10);
        }

        $this->SetXY($x + 2, $y + 43);
        $this->Cell(75, 5, $kp_pengaturan['kepala_sekolah'], 0, 1, 'R');
		$this->SetXY($x + 2, $y + 46);
		$this->Cell(75, 5, 'NIP ' . $kp_pengaturan['nip_kepala_sekolah'], 0, 1, 'R');
    }
}
...
if ($kelas !== '') {
    $q = mysqli_query($conn, "SELECT * FROM kp_siswa WHERE kelas = '$kelas'");
} else {
    $q = mysqli_query($conn, "SELECT * FROM kp_siswa");
}
...
if (isset($_GET['nisn'])) {
    ...
    $q = mysqli_query($conn, "SELECT * FROM kp_siswa WHERE nisn = '$nisn'");
...
$kp_pengaturan = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM kp_pengaturan LIMIT 1"));

$pdf = new PDF_Kartu('P', 'mm', 'A4');
$pdf->SetMargins(0, 0);
$pdf->AddPage();

for ($i = 0; $i < count($kp_siswa); $i++) {
    $x = 10;
    $y = 10 + ($i % 4) * 70;
    $pdf->gambarDepan($x, $y, $kp_siswa[$i], $kp_pengaturan);
    $pdf->gambarBelakang($x + 90, $y, $kp_siswa[$i], $kp_pengaturan);

    if (($i + 1) % 4 == 0 && $i + 1 < count($kp_siswa)) {
        $pdf->AddPage();
    }
}
