    ...
    if (move_uploaded_file($_FILES['file']['tmp_name'], $target)) {
        ...
        if (($handle = fopen($target, 'r')) !== false) {
            ...
            while (($data = fgetcsv($handle, 1000, ",")) !== false) {
                ...
                $cek_nis = mysqli_query($conn, "SELECT * FROM kp_siswa WHERE nis='$nis'");
                $cek_nisn = mysqli_query($conn, "SELECT * FROM kp_siswa WHERE nisn='$nisn'");
                $cek_user = mysqli_query($conn, "SELECT * FROM kp_users WHERE username='$nisn'");
                ...
                if ($insert) {
                    ...
                    echo "<p>❌ Baris $i: Gagal insert ke tabel kp_siswa.</p>";
                    $fail++;
                }
                $i++;
            }
